...
func (uc UserController) getUser(response http.ResponseWriter, request *http.Request, p httprouter.Params) {
	response.Header().Add("content-type", "application/json")
	var UserArray []Users
	ctx, _ := context.WithTimeout(context.Background(), 10*time.Second)
...
func (pc PostsController) getPost(response http.ResponseWriter, request *http.Request, parameters httprouter.Params) {
	response.Header().Add("content-type", "application/json")
	var postArray []Posts
	ctx, _ := context.WithTimeout(context.Background(), 10*time.Second)
